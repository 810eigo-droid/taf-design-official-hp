...
## 記事05 URL変更確認（2026-09-26）
ユーザーがスラッグをmembership-site-planningへ変更。公開APIと新URLのHTTP 200を確認。旧日本語URLから新URLへ転送されることも確認済み。記事05正規URL: https://taf-design.com/membership-site-planning/ 。以前の日本語URL維持の案内より本節を優先。Git内の固定リンクと配布HTMLを更新。本番HPの固定リンク更新は未反映（既存リンクも転送で閲覧可能）。

## 2026-09-26 カテゴリー別の自動一覧
WPの公開記事とカテゴリーを各100件ずつ全ページ取得。記事があるカテゴリーのみ見出しと全記事リンクを表示。PC3列・スマホ2列、折りたたみ・30件上限なし。複数カテゴリーの記事は各該当分類に掲載。新カテゴリー・新記事はWPで設定して公開すれば自動反映。新記事ラベルは投稿タイトル、既存5記事は短いラベルを維持。通信失敗時は静的な初期一覧を保持。初期一覧は現在の3カテゴリー・5記事を収録。重複した取得スクリプトも1本に統一。本番反映は未実施。
