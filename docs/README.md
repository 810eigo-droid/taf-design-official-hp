...
> **【2026-08-25】リポジトリ統一により、全体の司令塔はルートの [README.md](../README.md) になりました。まずそちらを見てください。**


> AI・自動化エージェントは、作業前にリポジトリ直下の `AGENTS.md` を必ず確認すること。ブログのSEO・AI検索対策とWordPress納品項目は同ファイルを最上位ルールとする。
...
- **ブログはHTMLだけで完了にしない**: 投稿タイトル、スラッグ、SEOタイトル、ディスクリプション、robots、canonical、OG画像、構造化データ、公開前後チェックを必ずセットで共有する。詳細はルートの `AGENTS.md` を参照
